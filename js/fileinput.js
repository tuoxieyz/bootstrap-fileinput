...
(function (factory) {
    ...
    FileInput.prototype = {
        ...
        _init: function (options, refreshMode) {
            ...
            self.uploadFileAttr = !$h.isEmpty($el.attr('name')) ? $el.attr('name') : 'file'; //'file_data'
        ...
        _initResumableUpload: function () {
            ...
            rm = self.resumableManager = {
                ...
                processUpload: function () {
                    ...
                    fnBefore = async function (jqXHR) {
                        outData = await self._getOutData(fd, jqXHR);
                        self._raise('filetestbeforesend', [id, fm, rm, outData]);
                    };
                    fnSuccess = async function (data, textStatus, jqXHR) {
                        outData = await self._getOutData(fd, jqXHR, data);
                    ...
                    fnError = async function (jqXHR, textStatus, errorThrown) {
                        outData = await self._getOutData(fd, jqXHR);
                        self._raise('filetestajaxerror', [id, fm, rm, outData]);
                        rm.setAjaxError(jqXHR, textStatus, errorThrown, true);
                        rm.uploadResumable();
                    };
                    fnComplete = async function () {
                        self._raise('filetestcomplete', [id, fm, rm, await self._getOutData(fd)]);
                    };
                ...
                sendAjax: function (index, retry, deferrer) {
                    ...
                    fnComplete = async function () {
                        if (!self._isAborted()) {
                            self._raise('filechunkcomplete', [id, index, retry, fm, rm, await self._getOutData(fd)]);
                        }
                    };
        ...
        _submitForm:async function () {
            var self = this;
            return self._isFileSelectionValid() && !await self._abort({});
        },
        ...
        _getOutData: async function (formdata, jqXHR, responseData, filesData) {
            var self = this;
            jqXHR = jqXHR || {};
            ...
            return {
                ...
                extra: await self._getExtraData(),
        ...
        _getExtraData: async function (fileId, index) {
            var self = this, data = self.uploadExtraData;
            if (typeof self.uploadExtraData === 'function') {
                data = await self.uploadExtraData(fileId, index);
            }
            return data;
        },
        ...
        _ajaxSubmit: async function (fnBefore, fnSuccess, fnComplete, fnError, formdata, fileId, index, vUrl) {
            ...
            data = await self._getExtraData(fileId, index) || {};
            if (typeof data === 'object') {
                $.each(data, function (key, value) {
                    formdata.append(key, value);
                });
            }
            self._setUploadData(formdata, { fileId: fileId });
            formdata.append(self.uploadFileAttr, formdata.fileObj.file, formdata.fileObj.fileName);           
            defaults = {
                xhr: function () {
        ...
        _uploadSingle: async function (i, id, isBatch) {
            ...
            chkComplete = function () {
                ...
                setTimeout(async function () {
                    ...
                    self._raise('filebatchuploadcomplete', [fm.stack, await self._getExtraData()]);
            ...
            fnBefore = async function (jqXHR) {
                outData = await self._getOutData(formdata, jqXHR);
                fm.initStats(id);
            ...
            fnSuccess = async function (data, textStatus, jqXHR) {
                ...
                outData = await self._getOutData(formdata, jqXHR, data);
                $.extend(true, params, outData);
                setTimeout(function () {
            ...
            fnError = function (jqXHR, textStatus, errorThrown) {
                ...
                setTimeout(async function () {
                    ...
                    $.extend(true, params, await self._getOutData(formdata, jqXHR));
            ...
            formdata.fileObj = { file: fileObj.file, fileName: fileName };
            self._ajaxSubmit(fnBefore, fnSuccess, fnComplete, fnError, formdata, id, i);
        },
        _uploadBatch:async function () {
            var self = this, fm = self.fileManager, total = fm.total(), params = {}, fnBefore, fnSuccess, fnError,
                fnComplete, hasPostData = total > 0 || !$.isEmptyObject(self.uploadExtraData), errMsg,
                setAllUploaded, formdata = new FormData(), op = self.ajaxOperations.uploadBatch;
            if (total === 0 || !hasPostData || await self._abort(params)) {
                return;
            }
            ...
            fnBefore = async function (jqXHR) {
                self.lock();
                fm.initStats();
                var outData = await self._getOutData(formdata, jqXHR);
                self.ajaxAborted = false;
                if (self.showPreview) {
                    self._getThumbs().each(function () {
            ...
            fnSuccess = async function (data, textStatus, jqXHR) {
                ...
                var outData = await self._getOutData(formdata, jqXHR, data), key = 0,
            ...
            fnComplete = async function () {
                ...
                self._initSuccessThumbs();
                self._clearFileInput();
                self._raise('filebatchuploadcomplete', [self.fileManager.stack, await self._getExtraData()]);
            };
            fnError = async function (jqXHR, textStatus, errorThrown) {
                var outData = await self._getOutData(formdata, jqXHR);
        ...
        _uploadExtraOnly:async function () {
            ...
            fnSuccess = async function (data, textStatus, jqXHR) {
                var outData = await self._getOutData(formdata, jqXHR, data);
                if ($h.isEmpty(data) || $h.isEmpty(data.error)) {
                    self._raise('filebatchuploadsuccess', [outData]);
                    self._clearFileInput();
                    self._initUploadSuccess(data);
        ...
        _change: function (e) {
            ...
            var $el = self.$element, isDragDrop = arguments.length > 1, isAjaxUpload = self.isAjaxUpload,
                ...
                throwError = async function (mesg, file, previewId, index) {
                    var p1 = $.extend(true, {}, await self._getOutData(null, {}, {}, files), { id: previewId, index: index }),
                        p2 = { id: previewId, index: index, file: file, files: files };
                    self.isPersistentError = true;
                    return isAjaxUpload ? self._showFileError(mesg, p1) : self._showError(mesg, p2);
                },
                maxCountCheck = async function (n, m, all) {
                    var msg = all ? self.msgTotalFilesTooMany : self.msgFilesTooMany;
                    msg = msg.replace('{m}', m).replace('{n}', n);
                    self.isError = await throwError(msg, null, null, null);
                    self.$captionContainer.removeClass('icon-visible');
        ...
        _abort: async function (params) {
            var self = this, data;
            if (self.ajaxAborted && typeof self.ajaxAborted === 'object' && self.ajaxAborted.message !== undefined) {
                data = $.extend(true, {}, await self._getOutData(null), params);
        ...
        readFiles: function (files) {
            ...
            var self = this, reader = self.reader, $container = self.$previewContainer,
                ...
                throwError = async function (msg, file, previewId, index, fileId) {
                    var $thumb, p1 = $.extend(true, {}, await self._getOutData(null, {}, {}, files),
        ...
        lock: async function (selectMode) {
            ...
            self._raise('filelock', [self.fileManager.stack, await self._getExtraData()]);
            return self.$element;
        },
        unlock: async function (reset) {
            var self = this, $container = self.$container;
            if (reset === undefined) {
                reset = true;
            }
            ...
            self._raise('fileunlock', [self.fileManager.stack, await self._getExtraData()]);
            return self.$element;
        },
        resume: function () {
        ...
        upload: async function () {
            var self = this, fm = self.fileManager, totLen = fm.count(), i, outData,
                hasExtraData = !$.isEmptyObject(await self._getExtraData());
            ...
            if (self.uploadAsync || self.enableResumableUpload) {
                outData = await self._getOutData(null);
                self._raise('filebatchpreupload', [outData]);
                self.fileBatchCompleted = false;
                self.uploadCache = [];
